...
use mach_sys::{
    kern_return_t, mach_port_deallocate, mach_port_t, mach_task_self,
...
// Call into libhurduser's `fsys_startup` MIG stub. Linked at the
// workspace level via `-lhurduser` in `.cargo/config.toml`, which
// puts the canonical mig-generated client routine in our binary's
// symbol table. By going through this exposed C function we
// sidestep the hand-rolled marshalling entirely and let mig's stub
// produce the wire bytes — useful as a known-good baseline when
// our own MIG attempts misbehave.
extern "C" {
    fn fsys_startup(
...
pub unsafe extern "C" fn trivfs_startup(
    ...
    // the fsys_startup handshake via libhurduser's mig-generated stub.
